...
| P5 | 体验对齐油猴 | 🚧 关注/关键词已完成 | 已读淡化、书签等继续补齐 |
...
| 项 | 状态 | 说明 |
|---|---|---|
| 关注名单同步 | ✅ | 登录后按账号缓存，每天自动同步一次，也可在设置中手动刷新 |
| 关注作者高亮 | ✅ | 主题列表识别接口返回的参与作者；详情按每层作者高亮，并显示“已关注”徽章 |
| 关键词高亮 | ✅ | 仅匹配主题标题、不区分大小写；支持多规则、颜色和独立开关 |
| 已读淡化 / 书签高亮 | ⏳ | 后续继续与 userscript 对齐 |
| 其他状态与快捷功能 | ⏳ | 按原生 macOS 交互逐项移植 |
...
### 2026-07-18 · 关注作者与关键词高亮

- 按账号请求 `/u/{username}/follow/following.json`，每天自动同步一次关注名单，并支持手动刷新。
- 主题列表解析 Discourse 返回的楼主与参与作者；详情楼层按当前作者匹配关注名单。
- 高亮使用原生语义背景、左侧强调线和“已关注”文字徽章，自动适配浅色/深色与选中态。
- 修正 Discourse `user_id` 等缩写字段的解码；关注与关键词同时命中时，绿色关注背景优先，关键词徽章仍保留。
- 设置页支持关注高亮开关和颜色，以及多条标题关键词规则、独立颜色、启用与删除。
- 关键词匹配语义与 userscript 一致：只匹配主题标题、不区分大小写，第一条命中规则优先。
- 版本 **0.7.0**。

### 2026-07-18 · 详情滚动与长帖性能优化

- 将详情区从“每层一个 WKWebView”收敛为整个主题共用一个 WKWebView，消除正文中央滚轮被内层视图吞掉的问题。
- 楼层、图片、onebox、关注作者高亮和回复入口在同一连续文档中渲染，减少长帖滚动时的布局与图层合成开销。
- 加载更多楼层时保存并恢复当前位置，避免正文跳回顶部。
- 正文与登录宿主共用 WebKit 数据存储和兼容 UA，使等级受限内容中的受保护图片沿用当前登录会话。

### 2026-07-18 · 手动列表刷新与侧边栏设置

- 列表不设置定时器或轮询；登录状态变化只提示手动刷新，不立即发送列表请求。
- 列表支持原生下拉刷新，刷新指示器会等待真实请求完成；刷新期间重复操作不会重发请求。
- 移除主容器与列表视图之间重复的首次加载入口，避免视图生命周期竞争触发冗余请求。
- 完整设置页作为原生侧边栏目的地展示，并适配默认窗口和最小窗口；系统标准设置窗口继续保留。
